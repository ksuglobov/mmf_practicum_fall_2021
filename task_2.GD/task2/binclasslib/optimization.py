...
class GDClassifier:
    ...
    def fit(self, X, y, w_0=None, trace=True, dataset=None):
        ...
        for self.n_iter in trange(0, self.max_iter + 1):
            timer = time()
            if self.n_iter > 0:
                self.step(X, y)
            ...
            if (self.n_iter > 0
                    and abs(loss_arr[-1] - loss_arr[-2]) < self.tol):
                break
        if trace:
    ...
    def init_w(self, n):
        bound = 1 / (2 * n)
        return np.random.default_rng(self.seed).uniform(-bound, bound, size=n)
...
class SGDClassifier(GDClassifier):
    ...
    def __init__(self, loss_function='binary_logistic', batch_size=500, step_alpha=1,
    ...
    def fit(self, X, y, w_0=None, trace=True, log_freq=1, dataset=None):
        """
        Обучение метода по выборке X с ответами y
        X - scipy.sparse.csr_matrix или двумерный numpy.array
        y - одномерный numpy array
